...
static int initDisplay(struct Engine *engine) {
    ...
    checkEGLApiErrors("eglCreateWindowSurface");
    MyLOGI("surface = %p\n", surface);

    context = eglCreateContext(display, config, EGL_NO_CONTEXT, ctxAttr);
    checkEGLApiErrors("eglCreateContext");
    MyLOGI("context = %p\n", context);
...
void android_main(struct android_app *app) {
    ...
    while (engine.isAlive) {
        ...
        while ((ident = ALooper_pollOnce(engine.playState == STATE_PREPARED ? 0 : -1, NULL, &events,
                                        (void **) &source)) >= 0) {
            MyLOGI("android_main while ALooper_pollAll %d", events);
            // Process this event.
            if (source != NULL) {
        ...
        if (render.isAlive) {
            MyLOGI("android_main while engine.animating");
